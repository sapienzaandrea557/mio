...
SCRCPY_DIR  = os.path.join(BASE_DIR, "bin", "scrcpy-win64-v3.3.4")
SCRCPY_PATH = os.path.join(SCRCPY_DIR, "scrcpy.exe")
ADB_PATH    = os.path.join(SCRCPY_DIR, "adb.exe")

def detect_os(ip):
    """Rileva OS tramite TTL e banner grabbing"""
    try:
        # metodo 1: TTL dal ping
        ...
        # metodo 2: banner grabbing porta 80/445/22
        for porta, label in [(445, "Windows"), (22, "Linux"), (80, None)]:
            try:
...
def open_gestisci(ip):
    ...
    tk.Label(action_frame, text="TRASFERISCI",
    ...
    def ricevi_file():
        ...
        if not s:
            messagebox.showerror("Errore", "Dispositivo non connesso via adb.")
            return
        # mostra file sul telefono
        win2 = tk.Toplevel(win)
        win2.title("Seleziona file dal telefono")
        win2.geometry("500x400")
        win2.configure(bg=BG)
        win2.grab_set()

        tk.Label(win2, text="File in /sdcard/Download/",
        ...
        def scarica():
            ...
            r = subprocess.run([ADB_PATH, "-s", s, "pull",
                              ...
                              capture_output=True, text=True)
            if r.returncode == 0:
                messagebox.showinfo("OK", f"File salvato in {dest}/{nome}")
                win2.destroy()
            else:
                messagebox.showerror("Errore", r.stderr.strip()[:100])
    ...
    def installa_apk():
        ...
        if "Success" in r.stdout:
            messagebox.showinfo("OK", "APK installata con successo!")
        else:
        ...
        def disinstalla():
            ...
            if messagebox.askyesno("Conferma", f"Disinstallare {pkg}?"):
                r = subprocess.run([ADB_PATH, "-s", s, "uninstall", pkg],
                                  capture_output=True, text=True)
                if "Success" in r.stdout:
                    messagebox.showinfo("OK", f"{pkg} disinstallata!")
                    aggiorna()
                else:
    ...
    tk.Button(trasf_row, text="📤  INVIA FILE",
              ...
              cursor="hand2", command=invia_file).pack(side="left", padx=4)

    tk.Button(trasf_row, text="📥  RICEVI FILE",
              font=("Courier New", 10, "bold"),
              bg=SURFACE, fg=GREEN, relief="flat", padx=12, pady=6,
              cursor="hand2", command=ricevi_file).pack(side="left", padx=4)

    tk.Button(trasf_row, text="📦  INSTALLA APK",
              font=("Courier New", 10, "bold"),
              bg=SURFACE, fg=YELLOW, relief="flat", padx=12, pady=6,
              cursor="hand2", command=installa_apk).pack(side="left", padx=4)

    tk.Button(trasf_row, text="📋  GESTISCI APP",
              font=("Courier New", 10, "bold"),
              bg=SURFACE, fg=MUTED, relief="flat", padx=12, pady=6,
              cursor="hand2", command=lista_app).pack(side="left", padx=4)
...
class WifiScanner(tk.Tk):
    ...
    def _build_ui(self):
        ...
        cols = ("ip", "hostname", "mac", "os", "stato", "blocco")
        ...
        headers = {"ip": ("IP ADDRESS", 130), "hostname": ("HOSTNAME", 170), "os": ("OS", 110),
        ...
        self.auto_btn = tk.Button(btn_frame, text="🔌  CONNETTI",
                                   font=("Courier New", 11),
                                   bg=SURFACE, fg=YELLOW, activebackground=BORDER,
                                   relief="flat", padx=20, pady=8, cursor="hand2",
                                   command=self._connetti_auto)
        self.auto_btn.pack(side="left", padx=(10, 0))
