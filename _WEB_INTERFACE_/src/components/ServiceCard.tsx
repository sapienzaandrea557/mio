...
const ServiceCard = ({ icon, title, description, tags, color = "brand" }: ServiceCardProps) => {
  ...
  return (
    <div className="glass-card p-10 reveal-up group hover:border-brand/50 transition-all duration-500 rounded-3xl border border-white/20 bg-white/5 backdrop-blur-md">
      ...
      <ul className="space-y-2 text-sm font-bold opacity-30 group-hover:opacity-60 transition-opacity uppercase tracking-widest" aria-label={`Dettagli per ${title}`}>
        {tags.map((tag: string) => (
